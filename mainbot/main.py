...
class DeskBot(driver):
    ...
    def login_transition_load(self):
        self.wait.until(EC.url_to_be(const.BASE_URL))
        self.wait.until(lambda driver: driver.execute_script('return jQuery.active') == 0)
        self.wait.until(lambda driver: driver.execute_script('return document.readyState') == 'complete')
    ...
    def add_to_cart(self):
        # Waiting for website/Ajax to fully load
        self.wait.until(lambda driver: driver.execute_script('return jQuery.active') == 0)
        self.wait.until(lambda driver: driver.execute_script('return document.readyState') == 'complete')

        self.wait.until(EC.element_to_be_clickable((By.LINK_TEXT, const.item_cart)))
        add_to_cart = self.find_element_by_link_text(const.item_cart)
        add_to_cart.click()
    ...
    def checkout_non_member_fill_details(self):
        ...
        checkout_to_payment_page = self.find_element_by_id('btn_payment')
    ...
    def checkout_member_fill_details(self):
        ...
        member_address_1 = self.find_element_by_xpath(
            '/html/body/div/div[1]/div[3]/div[2]/div/table/tbody/tr[1]/td[1]/a[1]/span')
        member_address_1.click()
    ...
    def empty_cart_one_item(self, teardown=True):
        self.wait.until(EC.element_to_be_clickable((By.XPATH,'/html/body/main/div/div[1]/div[2]/div[2]/div/table/tbody/tr[1]/td[8]/a')))
        remove_one = self.find_element_by_xpath('/html/body/main/div/div[1]/div[2]/div[2]/div/table/tbody/tr[1]/td[8]/a')
        remove_one.click()
        time.sleep(1)
        switch = self.switch_to_alert()
        switch.accept()
    ...
    def delete_address(self):
        self.wait.until(EC.element_to_be_clickable((By.ID, 'allCheck')))
        select_check_box = self.find_element_by_id('allCheck')
        select_check_box.click()

        self.wait.until(EC.element_to_be_clickable((By.ID, 'delAddr')))
        del_address = self.find_element_by_id('delAddr')
        del_address.click()
